...
class ChannelPruningEnv:
    ...
    def _init_data(self):
        ...
        train_set = Dataset(self.data_type.x_train, self.data_type.y_train)
        self.train_loader = torch.utils.data.DataLoader(train_set, batch_size=32, shuffle=True)

        val_set = Dataset(self.data_type.x_val, self.data_type.y_val)
        self.val_loader = torch.utils.data.DataLoader(val_set, batch_size=32, shuffle=True)
        # self.train_loader, self.val_loader, n_class = \
        #
        #     get_split_dataset(self.data_type, self.batch_size,
        ...
        with torch.no_grad():
            for i_b, (input, target) in enumerate(self.train_loader):  # use image from train set
                if i_b == self.n_calibration_batches:
                    break
                self.data_saver.append((input, target))
                input_var = torch.autograd.Variable(input).cuda().float()
    ...
    def _validate(self, val_loader, model, verbose=False):
        ...
        with torch.no_grad():
            for i, (input, target) in enumerate(val_loader):
                ...
                target_var = torch.max(target_var, 1)[1]
